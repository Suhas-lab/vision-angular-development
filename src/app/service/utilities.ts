...
export class Utilities {
    ...
    /* This function is getting used to for file conversion to binary format */
    public static createBlob(file, callback) {
    ...
    /* check boolean value */
    public static booleanMethod(params: any): number {
        if (params) {
            return 1;
        } else {
            return 0;
    ...
    /* check valid course duration value */
    public static courseTimeDuration(params: any): boolean {
        if (
            params[0] < 0 ||
            params[0] > 24 ||
            (params[1] < 0 || params[1] > 60)
        ) {
            return true;
        } else {
            return false;
    ...
    public static getHttpResponseMessage(
        ...
        if (data instanceof HttpResponseBase) {
            ...
            if (!responses.length && this.getResponseBody(data)) {
                responses.push(
                    `${data.statusText}: ${this.getResponseBody(
                        data,
                    ).toString()}`,
                );
            }
    ...
    public static printDate(date: Date, separator = 'at') {
        return `${Utilities.printDateOnly(
            date,
        )} ${separator} ${Utilities.printTimeOnly(date)}`;
    }
    ...
    public static printShortDate(
        ...
        return `${month}${separator}${day}${separator}${year} ${dateTimeSeparator} ${Utilities.printTimeOnly(
            date,
        )}`;
    }
